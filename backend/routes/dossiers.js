...
const fileFilter = (req, file, cb) => {
  const allowedMimes = ['application/pdf', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.ms-excel'];
...
const validateDossierData = (req, res, next) => {
  const { numero, vin, marque, modele, immatriculation, kilometrage, dateEntree } = req.body;
  
  if (!numero || !vin || !marque || !modele || !immatriculation || kilometrage === undefined || !dateEntree) {
    return res.status(400).json({
      message: 'Champs obligatoires manquants',
      required: ['numero', 'vin', 'marque', 'modele', 'immatriculation', 'kilometrage', 'dateEntree']
    });
  }
  
  // Validation basique VIN
  if (!/^[A-HJ-NPR-Z0-9]{17}$/.test(vin.toUpperCase())) {
    return res.status(400).json({ message: 'VIN invalide (17 caractères alphanumériques)' });
  }
  
  // Validation marque
  const marques = ['Volkswagen', 'SEAT', 'CUPRA', 'Škoda'];
  if (!marques.includes(marque)) {
    return res.status(400).json({ message: `Marque invalide. Acceptées: ${marques.join(', ')}` });
  }
      ...
      if (req.files) {
        if (req.files['fichePedagogiqueFile']) {
          ...
          if (oldDossier?.fichePedagogiqueUrl) {
            const oldPath = path.join(__dirname, '..', oldDossier.fichePedagogiqueUrl);
        ...
        if (req.files['documentPdfFile']) {
          if (oldDossier?.documentPdfUrl) {
            const oldPath = path.join(__dirname, '..', oldDossier.documentPdfUrl);
            if (fs.existsSync(oldPath)) fs.unlinkSync(oldPath);
          }

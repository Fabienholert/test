...
router.post('/register', async (req, res) => {
  try {
    ...
    const emailResult = await sendVerificationEmail(email, verificationToken, prenom);
    if (!emailResult.success) {
      // L'utilisateur est créé mais l'email n'a pas pu être envoyé
      return res.status(500).json({ 
        error: 'Compte créé mais email de vérification non envoyé. Veuillez contacter le support.',

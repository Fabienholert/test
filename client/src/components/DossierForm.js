...
function DossierForm({ initialData = {}, onSubmit, onCancel, isLoading }) {
    ...
    if (initialData && Object.keys(initialData).length > 0) {
      setFormData({
        ...
        dateEntree: initialData.dateEntree
          ? new Date(initialData.dateEntree)
          : null,
        dateImpression: initialData.dateImpression
          ? new Date(initialData.dateImpression)
          : null,
        typeDossier: initialData.typeDossier || "Garantie",
        isDISS: initialData.isDISS || false,
        numDISS: Array.isArray(initialData.numDISS)
          ? initialData.numDISS
          : initialData.numDISS
            ? [initialData.numDISS]
            : [],
        ...
        dateFinGarantie: initialData.dateFinGarantie
          ? new Date(initialData.dateFinGarantie)
          : null,
  ...
  const handleDateChange = (name, date) => {
    ...
    if (name === "dateImpression" && date) {
      ...
      if (printDate > today) {
        alert(
          "Erreur : la date d'impression ne peut pas être une date future.",
        );
      ...
      if (formData.dateEntree) {
        ...
        if (printDate > entryDate) {
          alert(
            "Erreur : la date d'impression ne peut pas être après la date d'entrée du véhicule.",
          );
          return; // Bloque la sélection
        }
      }
    ...
    if (name === "dateEntree" && date && formData.dateImpression) {
      ...
      if (printDate > newEntryDate) {
        alert(
          "Erreur : la date d'impression déjà saisie serait après cette nouvelle date d'entrée.",
        );
        return; // Bloque la sélection
      }
    }
  ...
  const handleAnalyzePDF = async () => {
    if (!documentPdfFile) {
      setStatusMessage({
        type: "error",
        text: "Veuillez d'abord sélectionner un fichier PDF.",
      });
      return;
    }
      ...
      setFormData((prev) => ({
        ...
        dateEntree:
          data.dateEntree && data.dateEntree.includes("/")
            ...
            : prev.dateEntree,
        dateImpression:
          data.dateImpression && data.dateImpression.includes("/")
            ? new Date(data.dateImpression.split("/").reverse().join("-"))
            : prev.dateImpression,
        isDISS: data.isDISS || prev.isDISS,
        numDISS:
          data.numDISS && data.numDISS.length > 0 ? data.numDISS : prev.numDISS,
  ...
  const handleAnalyzeMCQ = async () => {
    if (!ficheMCQFile) {
      setStatusMessage({
        type: "error",
        text: "Veuillez d'abord sélectionner un fichier MCQ (PDF).",
      });
      return;
    }
    ...
    if (!formData.critere) {
      setStatusMessage({
        type: "error",
        text: "Veuillez renseigner le critère avant d'analyser la fiche MCQ.",
      });
      return;
    }
    ...
    setStatusMessage({
      type: "success",
      text: "Lancement de l'analyse OCR de la fiche MCQ...",
    });
    setIsAnalyzing(true);
    setFormData((prev) => ({ ...prev, lastExtractedText: "" }));
    ...
    const mqMatch = extractedText.match(
      /mesure\s*(?:de\s*)?qualit[éèe]\s*[:\-]?\s*([A-Z0-9]{4})\b/i,
    );
    if (mqMatch) {
      foundCode = mqMatch[1];
    } else {
      // Fallback si écrit différemment
      const fallbackAvarieMatch = extractedText.match(
        /(?:code(?:\s*d['']?avarie)?|action|campagne)\s*[:\-]?\s*([A-Z0-9]{4})\b/i,
      );
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes("directive de facture") || line.includes("activité")) {
        ...
        line.includes("pièces d'origine") ||
        line.includes("pièces requises")
      ) {
      ...
      if (critereRegex.test(line)) {
        ...
        if (inDirectiveFacture || line.includes("activité")) {
          ...
          if (
            activiteMatch &&
            activiteMatch[1].toLowerCase() !== formData.critere.toLowerCase() &&
            !foundMO
          ) {
        ...
        if (inPiecesOrigine) {
          ...
          const context = lines.slice(i, i + 2).join(" ");
          const pieceMatch = context.match(
            /\b([A-Z0-9]{3}\s?[A-Z0-9]{3}\s?[A-Z0-9]{3}(?:\s?[A-Z]{1,2})?)\b/i,
          );

          if (
            pieceMatch &&
            pieceMatch[1].toLowerCase() !== formData.critere.toLowerCase() &&
            !foundPieces
          ) {
            ...
            const potentialPiece = words.find(
              (w) =>
                w.length > 5 &&
                w.match(/[A-Z0-9]/i) &&
                w.toLowerCase() !== formData.critere.toLowerCase(),
            );
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.dateEntree) {
      const entryDate = new Date(formData.dateEntree);
      const referenceDate = initialData.dateCreation
        ? new Date(initialData.dateCreation)
        : new Date();
      ...
      if (diffDays > 24) {
        alert(
          "Erreur : la date d'entrée est supérieure de 24 jours à la date de création. Veuillez obligatoirement refaire le dossier.",
        );
        return;
      }
    }
    ...
    if (formData.dateImpression) {
      ...
      if (printDate > today) {
        alert(
          "Erreur : la date d'impression ne peut pas être une date future (après la date du jour).",
        );
        return;
      }
      ...
      {formData.lastExtractedText && (
        ...
        >
          <details>
            ...
            >
              📄 Voir le texte extrait du document (
              {formData.lastExtractedText.length} caractères)
            </summary>
            <div
              style={{
                marginTop: "1rem",
      ...
      <div className="form-group" style={{ marginBottom: "2rem" }}>
        ...
        <div style={{ display: "flex", gap: "1rem" }}>
          ...
          >
            ...
            <span
              style={{
                fontWeight: formData.typeDossier === "Garantie" ? "600" : "400",
              }}
            >
              Garantie
            </span>
          ...
          >
            ...
            <span
              style={{
                fontWeight: formData.typeDossier === "MCQ" ? "600" : "400",
              }}
            >
              MCQ (Rappel)
            </span>
            ...
            <div className="form-group">
              ...
              <label
                style={{
                  ...
                  cursor: "pointer",
                  userSelect: "none",
                  marginBottom: formData.isDISS ? "0.75rem" : 0,
                }}
              >
                <input
                  type="checkbox"
                ...
                <span
                  style={{
                    color: formData.isDISS
                      ? "var(--primary)"
                      : "var(--text-muted)",
                    fontWeight: formData.isDISS ? "600" : "400",
                    transition: "color 0.2s",
                  }}
                >
                  {formData.isDISS
                    ? `Oui — ${formData.numDISS.length} DISS`
                    : "Non"}
                </span>
              </label>
              {formData.isDISS && (
                ...
                >
                  {(formData.numDISS.length > 0 ? formData.numDISS : [""]).map(
                    (d, i) => (
                      <div
                        key={i}
                        style={{
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const updated = formData.numDISS.filter(
                              (_, idx) => idx !== i,
                            );
                            setFormData((prev) => ({
                              ...prev,
                              numDISS: updated,
                              isDISS: updated.length > 0,
                            }));
                          }}
                          style={{
            ...
            <div className="form-group">
              ...
              <label
                style={{
                  ...
                  cursor: "pointer",
                  userSelect: "none",
                  marginBottom: formData.isTPI ? "0.75rem" : 0,
                }}
              >
                <input
                  type="checkbox"
                ...
                <span
                  style={{
                    color: formData.isTPI
                      ? "var(--primary)"
                      : "var(--text-muted)",
                    fontWeight: formData.isTPI ? "600" : "400",
                    transition: "color 0.2s",
                  }}
                >
      ...
      <div className="grid grid-cols-2">
          ...
          <div className="form-group">
            ...
            <label
              style={{
                ...
                cursor: "pointer",
                userSelect: "none",
                marginBottom: formData.hasFichePedagogique ? "0.75rem" : 0,
              }}
            >
              <input
                type="checkbox"
              ...
              <span
                style={{
                  color: formData.hasFichePedagogique
                    ? "var(--primary)"
                    : "var(--text-muted)",
                  fontWeight: formData.hasFichePedagogique ? "600" : "400",
                  transition: "color 0.2s",
                }}
              >
            ...
            {formData.hasFichePedagogique && (
              ...
              >
                ...
                >
                  {initialData.fichePedagogiqueUrl
                    ? "Remplacer le fichier actuel"
                    : "Joindre la fiche pédagogique (PDF, Image...)"}
                </p>
              </div>
            )}
          </div>
        ...
        {formData.typeDossier === "MCQ" && (
          <div className="form-group">
            ...
            >
              ...
              >
                {initialData.ficheMCQUrl
                  ? "Remplacer le fichier actuel"
                  : "Joindre la fiche MCQ (PDF, Image...)"}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="form-group">
          ...
          <label
            style={{
              ...
              cursor: "pointer",
              userSelect: "none",
              marginBottom: formData.isPointageVerifie ? "0.75rem" : 0,
            }}
          >
            <input
              type="checkbox"
            ...
            <span
              style={{
                color: formData.isPointageVerifie
                  ? "var(--primary)"
                  : "var(--text-muted)",
                fontWeight: formData.isPointageVerifie ? "600" : "400",
                transition: "color 0.2s",
              }}
            >
        ...
        <div className="form-group">
          ...
          >
            ...
            >
              {initialData.documentPdfUrl
                ? "Remplacer le document PDF"
                : "Joindre le document PDF officiel du dossier"}
            </p>
          </div>
        </div>
      </div>
